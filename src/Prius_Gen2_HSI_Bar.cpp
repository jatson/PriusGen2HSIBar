...
void loop() 
{
    ...
    {
        if(dpadLeft == 0) displayMode--;
        else displayMode++;

        if(displayMode < DM_MIN) displayMode = DM_MAX;
        if(displayMode > DM_MAX) displayMode = DM_MIN;
    ...
    {
        if(data[11] !=9999) injectorTiming = data[11];
        else if(rpm < 970) 
    ...
    // accelerator
    if (data[5] !=9999) throttlePedalRaw = data[5];

    // brake
    if (data[6] !=9999) brakePedal = data[6];

    // Drive mode
    if (data[8] !=9999) gearShift = data[8];

    // SOC
    if (data[9] !=9999) stateOfCharge = data[9];
    ...
    if ((((gearShift & 0x7) >= GS_DRIVE) || ((gearShift & 0x7) == GS_REVERSE)))
    {
        ...
        if ((throttlePedalRaw == 0) && (brakePedal >0))
        {
            ...
            if (kph >= 5 && kph <=16)  // speed between 5 and 16 km / h, little difference between D and B
            {
                E=0;
                P=0;
                if (brakePedal >16)    R=(kph -5) *16 /11 ;                 // if greater than 16 BRAKE fills as the speed from 5
                if (brakePedal <=16)   R=(kph -5) *brakePedal /11 ;            // if BRAKE lower fills in force as brake and speed
            }
            ...
            if (kph >= 16)   // exceeding 16km / hslower
            {
                E=0;
                P=0;
                if (brakePedal >16)  R=16;                // if greater than 16 BRAKE fills the entire bar
                if (brakePedal <=16) R=brakePedal;           // if BRAKE  fills as the force on the brake from scratch
            }
        }
        ...
        if ((throttlePedalRaw > 0) && (brakePedal == 0))
        {
            ...
            if (kph<50)
            {
                if (stateOfCharge<45) pointA=5;
                if (stateOfCharge>=45 && stateOfCharge <=56) pointA=5+ ((stateOfCharge-45) *15 /11);
                if (stateOfCharge>=57 && stateOfCharge <67)  pointA = 20;
                if (stateOfCharge>=67 && stateOfCharge <77)  pointA = 20 + (stateOfCharge-67);
                if (stateOfCharge>=77) pointA = 30;
            }

            if (kph>=50 && kph<63)
            {
                if (stateOfCharge<45) pointA=5;
                if (stateOfCharge>=45 && stateOfCharge <=56) pointA=  5+  (   (stateOfCharge-45)*(    136-(kph-50)    *75  /10) /100);
                if (stateOfCharge>=57 && stateOfCharge <67 ) pointA = 5+  ((11)*(136-(kph-50)*75 /10) /100);
                if (stateOfCharge>=67 && stateOfCharge <77 ) pointA = (19+ (stateOfCharge-66))*(26-(kph-50))/26;
                if (stateOfCharge>=77) pointA = 15+ ((11)*(136-(kph-50)*105/10) /100);
            }

            if (kph>=63)
            {
                if (stateOfCharge<45) pointA=5;
                if (stateOfCharge>=45 && stateOfCharge <=56) pointA=5+ ((stateOfCharge-45) *5 /11);
                if (stateOfCharge>=57 && stateOfCharge <67 ) pointA = 10;
                if (stateOfCharge>=67 && stateOfCharge <77 ) pointA = 10+((stateOfCharge-67)/2);
                if (stateOfCharge>=77) pointA = 15;
            }
...
// function to draw an empty HSI bar
void clear_hsi(void)
{
    // Set up custom characters to form an empty HSI bar
    ...
    // Draw empty HSI bar custom characters
    sLCD.setCursor(0,0);
    sLCD.write((uint8_t)0);
    sLCD.write((uint8_t)1);
    sLCD.write((uint8_t)1);
    sLCD.write((uint8_t)5);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)4);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)3);
    sLCD.write((uint8_t)4);
    sLCD.write((uint8_t)6);
    sLCD.write((uint8_t)7);
}
...
void drawbar_E (int e)
{
    ...
    if (e==0)  // if 0 write all
    {
        if (EA !=0)
        {
            ...
            if (EA>=5)  sLCD.write((uint8_t)esc[1]);
            if (EA>=11) sLCD.write((uint8_t)esc[2]);
            if (EA>=17) sLCD.write((uint8_t)esc[3]);
            if (EA>=23) sLCD.write((uint8_t)esc[4]);
            if (EA>=29) sLCD.write((uint8_t)esc[5]);
            if (EA>=35) sLCD.write((uint8_t)esc[6]);
            if (EA>=41) sLCD.write((uint8_t)esc[7]);
            if (EA>=47) sLCD.write((uint8_t)esc[8]);
            if (EA>=53) sLCD.write((uint8_t)esc[9]);
            if (EA>=59) sLCD.write((uint8_t)esc[10]);
        }
    }
    else  // e!=0
    {
